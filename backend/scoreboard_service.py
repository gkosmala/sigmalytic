...
# Track which signals have already been logged this session
# symbol → last logged signal type
_logged_signals: dict[str, str] = {}
...
def log_signal(sym: dict, signal_type: str):
    ...
    Only logs if this is a new signal type for this symbol
    (prevents duplicate logging on every 60s scan).
    """
    symbol = sym.get("symbol", "")
    if not symbol or not DATABASE_URL:
        return

    # Don't re-log the same signal type for this symbol
    last = _logged_signals.get(symbol)
    if last == signal_type:
        return
    ...
    try:
        ...
        cur.close()
        conn.close()
        _logged_signals[symbol] = signal_type
...
def _grade_short(entry, target1, target2, invalidation, outcome) -> tuple[str, bool, bool, bool]:
    ...
    # For shorts: target1/target2 are below entry, invalidation is above entry
    hit_t2    = outcome <= target2      if target2 > 0 else False
    hit_t1    = outcome <= target1      if target1 > 0 else False
    hit_inval = outcome >= invalidation if invalidation > 0 else False
...
def grade_pending_signals():
    ...
    cutoff = datetime.now(timezone.utc) - timedelta(hours=20)  # ~1 trading day
    ...
    for row in pending:
        ...
        try:
            ...
            conn.close()
            graded += 1
            log.info(f"Graded {symbol} {sig_type}: {grade} (entry ${entry} → outcome ${outcome}, {outcome_pct:+.1f}%)")
        except Exception as e:
