...
def _fetch_alpaca_options(symbol: str, current_price: float) -> dict:
    ...
    try:
        ...
        # ── Step 1: Fetch OI using TradingClient SDK ──────────────────────────
        # OI is a static trading asset attribute — requires TradingClient
        # NOT OptionHistoricalDataClient (different infrastructure)
        # Updated once per morning after OCC clearing (~8:30 AM ET)
        oi_map = {}
        try:
            ...
            contracts_resp = trading_client.get_option_contracts(oi_req)
            for c in contracts_resp.option_contracts:
                oi_val = int(c.open_interest or 0)
                if c.symbol and oi_val > 0:
                    oi_map[c.symbol] = oi_val
            log.info(f"GEX {symbol}: OI fetched for {len(oi_map)} contracts via TradingClient")
