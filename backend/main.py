...
def get_candles(symbol: str, timeframe: str = "5Min", limit: int = 200):
    ...
    params = {
        "timeframe": timeframe,
        "limit": _clean_limit,
        # The live quote strip and stream both use SIP. Historical OHLCV
        # must use that same feed so the chart's bars match those trades.
        "feed": "sip",
        "adjustment": "raw",
...
RADAR_CONFIG_DEFAULTS = {
    # ADDED (2026-09-21): defaults deliberately match today's actual,
    # unconfigured production behavior exactly -- score_weis_wave_radar()'s
    # existing Spring/Upthrust/Buying Climax/Selling Climax/3-Bar
    # Reversal set, on daily bars, with the same 252-bar (~1 trading
    # year) lookback fetch_bars_batch() already uses by default. An
    # operator who never touches this new config sees no change at all.
...
def get_radar_config():
    """
    ADDED (2026-09-21): the real, operator-facing config for which
    signals the radar computes/displays, at what timeframe and
    lookback, and which of those signals (a possibly narrower set)
    should actually trigger an alert -- and via which channel(s).
    Public GET (read-only, no admin needed) so the settings UI and the
    scan worker can both read the current, live configuration.
    """
    try:
        import redis as _redis_lib
        import os as _os
...
def debug_signal_test(symbols: str = "AAPL,MSFT,TSLA,NVDA,AMD,ABNB,AA,AZO",
    ...
    """
    try:
        from backend.radar_service import fetch_bars_batch, trim_incomplete_bar, compute_symbol_signals, min_bars_required_for, weis_wave_threshold_for_timeframe
        ...
        # window buffer. Now shared with the real radar scan via
        # min_bars_required_for(), so the two can't drift apart on
        # this rule the way score_symbol()'s composite once did.
        min_bars_required = min_bars_required_for(requested_signals)
        ...
        for sym in sym_list:
            ...
            # ADDED (2026-09-21): now calls the same, shared
            # compute_symbol_signals() the real radar scan itself
            # calls -- extracted here unchanged from this endpoint's
            # own, already-proven logic, so this endpoint and
            # production can never silently diverge again.
            found = compute_symbol_signals(sym, bars, requested_signals, timeframe, wyckoff_engine=wyckoff_engine)
            results.append({"symbol": sym, "bar_count": len(bars), "signals_found": found})
