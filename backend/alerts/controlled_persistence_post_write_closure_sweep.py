...
one-row append-only audit insert and the D3E.7 readback verification.
...
def validate_d3e8_closure_row(row: Optional[Mapping[str, Any]]) -> Dict[str, Any]:
    if not row:
        return {
            "row_found": False,
            "closure_verified": False,
            "closure_status": "D3E8_EXPECTED_CONTROLLED_AUDIT_ROW_NOT_FOUND",
    ...
    closure_verified = all(expected_checks.values())

    return {
        "row_found": True,
        "closure_verified": closure_verified,
        "closure_status": (
            "D3E8_POST_PERSISTENCE_CLOSURE_VERIFIED_READ_ONLY"
            if closure_verified
            else "D3E8_CONTROLLED_AUDIT_ROW_FOUND_BUT_FIELD_MISMATCH"
        ),
        "inserted_row_id": row.get("id"),
        "inserted_row_created_at": row.get("created_at"),
        "closure_symbol": row.get("symbol"),
        "closure_audit_component": row.get("audit_component"),
        "closure_audit_version": row.get("audit_version"),
        "closure_operator_control_evidence_audit_status": row.get(
            "operator_control_evidence_audit_status"
        ),
        "closure_d3d_dry_run_gate_audit_status": row.get("d3d_dry_run_gate_audit_status"),
        "expected_checks": expected_checks,
    }


def build_d3e8_post_persistence_closure_sweep_payload(
    ...
    base_response: Dict[str, Any] = {
        ...
        "route_status": "D3E8_POST_PERSISTENCE_CLOSURE_SWEEP_ROUTE_READY",
        "route_is_mounted": True,
        "read_only": True,
        "closure_scope": [
            "D3E4 schema existence verified",
            "D3E5 hard-block route verified",
            "D3E6 one-row append-only audit insert executed",
            "D3E7 post-write readback verified",
            "D3E8 no-drift closure sweep",
        ],
    ...
    if not execute_live_read:
        return {
            **base_response,
            ...
            "read_status": "DRY_RUN_CLOSURE_PREVIEW_ONLY",
            "row_found": False,
            "closure_verified": False,
            "closure_status": "D3E8_DRY_RUN_CLOSURE_PREVIEW_ONLY",
        }
    ...
    if not supabase_url or not supabase_key:
        return {
            **base_response,
            ...
            "row_found": False,
            "closure_verified": False,
            "closure_status": "D3E8_SUPABASE_ENV_MISSING",
        }
    ...
    try:
        ...
        return {
            **base_response,
            ...
            "row_found": False,
            "closure_verified": False,
            "closure_status": "D3E8_READ_FAILED_HTTP_ERROR",
        }
        ...
        return {
            **base_response,
            ...
            "row_found": False,
            "closure_verified": False,
            "closure_status": "D3E8_READ_FAILED_EXCEPTION",
        }
    ...
    validation = validate_d3e8_closure_row(row)
