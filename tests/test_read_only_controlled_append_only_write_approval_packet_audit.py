...
from backend.alerts.controlled_append_only_write_approval_packet_audit import (
    build_read_only_controlled_append_only_write_approval_packet_from_preflight,
)
def test_controlled_append_only_write_approval_packet_blocks_when_preflight_blocked_no_drift():
    preflight = {
        ...
        "append_only_controls": {"append_only_declared": True},
        "append_only_preflight_requirements": ["explicit_human_approval_still_required"],
        "absolute_preflight_prohibitions": ["no_supabase_write_in_this_gate"],
        "explicit_human_approval_required_before_any_write": True,
        "supabase_append_only_write_preflight_authorization_gate_rows": [
            {
                "symbol": "SPY",
    ...
    result = build_read_only_controlled_append_only_write_approval_packet_from_preflight(
        preflight=preflight,
    )
    assert result["ok"] is True
    assert result["component"] == "CONTROLLED_APPEND_ONLY_WRITE_APPROVAL_PACKET_AUDIT_READ_ONLY"
    ...
    assert result["controlled_append_only_write_approval_packet_audit_status"] == "CONTROLLED_APPEND_ONLY_WRITE_APPROVAL_PACKET_BLOCKED_READ_ONLY"
    assert result["approval_packet_blocked_symbols"] == ["SPY", "QQQ"]
    assert result["controlled_append_only_write_approval_packet_applies_no_changes"] is True
    assert result["controlled_append_only_write_approval_packet_is_read_only"] is True
    assert result["controlled_append_only_write_approval_packet_never_writes"] is True
    assert result["controlled_append_only_write_approval_packet_never_authorizes"] is True
def test_controlled_append_only_write_approval_packet_complete_but_not_authorized_no_drift():
    preflight = {
    ...
    assert result["controlled_append_only_write_approval_packet_audit_status"] == "CONTROLLED_APPEND_ONLY_WRITE_APPROVAL_PACKET_COMPLETE_BUT_NOT_AUTHORIZED_READ_ONLY"
    assert result["approval_packet_hypothetically_complete_symbols"] == ["SPY"]
    assert result["approval_packet_authorized"] is False
    assert result["approval_packet_write_authorized"] is False
    assert result["append_only_write_preflight_authorized"] is False
    assert result["append_only_write_preflight_gate_clear"] is False
...
if __name__ == "__main__":
    test_controlled_append_only_write_approval_packet_blocks_when_preflight_blocked_no_drift()
    test_controlled_append_only_write_approval_packet_complete_but_not_authorized_no_drift()
    print("CONTROLLED_APPEND_ONLY_WRITE_APPROVAL_PACKET_AUDIT_MANUAL_TESTS_PASS")
